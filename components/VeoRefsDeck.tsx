...
import { Video, Loader2, Upload, Image as ImageIcon, Trash2, Palette } from 'lucide-react';
...
interface VeoRefsDeckProps {
    projectState?: ProjectState | null;
    userGallery?: string[];
}

export const VeoRefsDeck: React.FC<VeoRefsDeckProps> = ({ projectState, userGallery = [] }) => {
  const [prompt, setPrompt] = useState('');
  const [model, setModel] = useState<VeoModel>(VeoModel.QUALITY); // Default to Quality for Refs
  const [aspectRatio, setAspectRatio] = useState<string>('16:9');
  const [isGenerating, setIsGenerating] = useState(false);
  const [videoUrl, setVideoUrl] = useState<string | null>(null);
  
  // 3 Style/Object References
  const [ref1, setRef1] = useState<string | null>(null);
  const [ref2, setRef2] = useState<string | null>(null);
  const [ref3, setRef3] = useState<string | null>(null);
  ...
  const setReferenceToFirstEmpty = (img: string) => {
      if (!ref1) setRef1(img);
      else if (!ref2) setRef2(img);
      else if (!ref3) setRef3(img);
      else setRef1(img); // Overwrite first if full
  };
  ...
  const handleGenerate = async () => {
    ...
    const refs = [ref1, ref2, ref3].filter(Boolean) as string[];
  ...
  const renderUploadBox = (label: string, value: string | null, setter: (v: string | null) => void) => (
      <div className="space-y-1">
          ...
          <div className="relative aspect-square bg-slate-900 border border-dashed border-slate-700 rounded flex items-center justify-center overflow-hidden hover:border-amber-500/50 transition-colors cursor-pointer group">
              {value ? (
                  <>
                    <img src={resolveSrc(value)} className="w-full h-full object-cover" alt={label} />
                    <button 
                        onClick={(e) => { e.stopPropagation(); setter(null); }}
                        className="absolute top-1 right-1 bg-black/80 hover:bg-red-600 text-white rounded p-1 transition-all opacity-0 group-hover:opacity-100 z-10"
                    >
  ...
  const renderGalleryAsset = (img: string, i: number) => (
    <div key={i} className="group relative rounded overflow-hidden border border-slate-800 hover:border-amber-500 transition-all cursor-pointer" onClick={() => setReferenceToFirstEmpty(img)}>
        <img src={resolveSrc(img)} className="w-full h-auto" />
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
            <span className="text-[10px] text-white font-bold bg-amber-600 px-2 py-1 rounded">USE AS REF</span>
        </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full">
      ...
      <div className="lg:col-span-3 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
        <h2 className="text-lg font-bold text-white flex items-center gap-2"><Palette className="w-5 h-5 text-amber-500" /> Refs-to-Video</h2>
        ...
        <div className="space-y-2 pt-2 border-t border-slate-800">
            <label className="text-xs font-semibold text-slate-400 uppercase">Style/Object Ingredients (Max 3)</label>
            <div className="grid grid-cols-3 gap-2">
                {renderUploadBox("Ref 1", ref1, setRef1)}
                {renderUploadBox("Ref 2", ref2, setRef2)}
                {renderUploadBox("Ref 3", ref3, setRef3)}
            </div>
        </div>
        ...
        <button
          ...
          className={`w-full py-4 rounded-lg font-bold text-lg flex items-center justify-center gap-2 transition-all ${
            isGenerating 
              ...
              : 'bg-gradient-to-r from-pink-600 to-purple-500 hover:from-pink-500 hover:to-purple-400 text-white'
          }`}
        >
      ...
      <div className="lg:col-span-6 flex items-center justify-center bg-black rounded-xl border border-slate-800 overflow-hidden relative min-h-[400px]">
          ...
          {isGenerating && (
              <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-10">
                  ...
                  <p className="text-pink-500 font-mono animate-pulse">Rendering on Veo 3.1...</p>
              </div>
          )}
      </div>
